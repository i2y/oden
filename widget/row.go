...
func (r *RowLayout) style() string {
	return fmt.Sprintf("display: flex; flex-direction: row; flex: 1 1 0; width: 100%%; height: 100%%;")
}
...
func (r *RowLayout) layout() {
	for _, w := range r.children {
		...
		case FixedWidth:
			w.SetSizeStyle(fmt.Sprintf("flex: 0 0 %dpx; height: 100%%;", w.Width()))
		case FixedRatioWidth:
			w.SetSizeStyle(fmt.Sprintf("flex: 0 0 %d%%; height: 100%%;", w.Width()))
		case FixedHeight:
